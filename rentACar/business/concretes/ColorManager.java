...
public class ColorManager implements ColorService {
	...
	public void add(CreateColorRequest createColorRequest) {
		String colorName = createColorRequest.getColorName().toLowerCase();
		checkIfColorExists(colorName);
		createColorRequest.setColorName(colorName);

		/*
		 * if (colorDao.getByName(createColorRequest.getColorName()).size() == 0) {
		 * Color color = this.modelMapperService.forRequest().map(createColorRequest,
		 * Color.class); this.colorDao.save(color); }
		 */
	}
